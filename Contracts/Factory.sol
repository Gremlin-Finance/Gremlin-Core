...
contract Factory is IFactory {
    ...
    IGremlinPriceOracle public gremlinPriceOracle;
    ...
    constructor(
        ...
        IGremlinPriceOracle _gremlinPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        gremlinPriceOracle = _gremlinPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "Gremlin: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "Gremlin: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "Gremlin: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "Gremlin: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            gremlinPriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) gremlinPriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "Gremlin: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "Gremlin: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "Gremlin: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "Gremlin: UNAUTHORIZED");
