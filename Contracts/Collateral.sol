...
import "./interfaces/IGremlinCallee.sol";
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            IGremlinPriceOracle(gremlinPriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Gremlin: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Gremlin: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Gremlin: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(
            ...
            "Gremlin: UNAUTHORIZED"
        );

        (, uint256 shortfall) = accountLiquidity(borrower);
        require(shortfall > 0, "Gremlin: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        require(redeemAmount <= totalBalance, "Gremlin: INSUFFICIENT_CASH");

        // optimistically transfer funds
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            IGremlinCallee(redeemer).gremlinRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Gremlin: INSUFFICIENT_REDEEM_TOKENS"
        );
